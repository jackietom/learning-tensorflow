...
x = tf.constant([29.05088806,  27.61298943,  31.19073486,  29.35532951,
                 ...
                 27.88236427, 20.56035233, 30.20379066, 29.51215172,
                 33.71149445, 28.59134293, 36.05556488, 28.66994858])
idx = tf.where(tf.greater(x, tf.constant(30, dtype=tf.float32)))
...
condition = tf.less(tf.reduce_max(x-y), tf.constant(0, dtype=tf.float32))
output = tf.where(condition, mse, sav)
print(output.eval())
sess.close()